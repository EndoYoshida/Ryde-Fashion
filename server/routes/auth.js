...
async function generateUniqueUsername(base) {
  ...
  let username = clean;
  let i = 1;
  while (await db.prepare("SELECT id FROM customers WHERE LOWER(username) = ?").get(username)) {
    username = `${clean}${i++}`;
  }
  return username;
}
...
router.post("/firebase", asyncHandler(async (req, res) => {
  ...
  let customer = await db.prepare("SELECT * FROM customers WHERE firebase_uid = ?").get(uid);
  ...
  if (!customer) {
    ...
    customer = await db.prepare("SELECT * FROM customers WHERE email = ?").get(cleanEmail);

    if (customer) {
      await db.prepare("UPDATE customers SET firebase_uid = ?, email_verified = ? WHERE id = ?")
        .run(uid, emailVerified ? 1 : 0, customer.id);
    } else {
      const finalUsername = await generateUniqueUsername(username || name || cleanEmail.split("@")[0]);
      await db.prepare(`
        INSERT INTO customers (name, username, email, phone, firebase_uid, email_verified, joined, status)
        VALUES (?, ?, ?, ?, ?, ?, to_char(now(), 'YYYY-MM-DD'), 'active')
      `).run(name?.trim() || finalUsername, finalUsername, cleanEmail, phone?.trim() || null, uid, emailVerified ? 1 : 0);
    }
    customer = await db.prepare("SELECT * FROM customers WHERE firebase_uid = ? OR email = ?").get(uid, cleanEmail);
    ...
    await db.prepare("UPDATE customers SET email_verified = ? WHERE id = ?").run(emailVerified ? 1 : 0, customer.id);
...
router.patch("/me", requireCustomer, asyncHandler(async (req, res) => {
  ...
  const updated = await db.prepare("SELECT * FROM customers WHERE id = ?").get(req.customer.id);
...
router.post("/delete-account/request", requireCustomer, asyncHandler(async (req, res) => {
  if (!req.customer.email_verified) {
    return res.json({ requiresCode: false });
  }
  const code = generateVerificationCode();
  await db.prepare("UPDATE customers SET verification_code = ?, verification_code_expires_at = ? WHERE id = ?")
...
router.delete("/me", requireCustomer, asyncHandler(async (req, res) => {
  const { code } = req.body || {};
  ...
  await db.prepare("DELETE FROM customer_sessions WHERE customer_id = ?").run(req.customer.id);
...
router.get("/me/orders", requireCustomer, asyncHandler(async (req, res) => {
  const orders = await db.prepare("SELECT id, status, payment_status, total, date FROM orders WHERE email = ? ORDER BY date DESC")
    .all(req.customer.email);
  const withItems = await Promise.all(orders.map(async (o) => ({
    ...o,
    paymentStatus: o.payment_status,
    items: await db.prepare("SELECT name, qty, price FROM order_items WHERE order_id = ?").all(o.id),
  })));
  res.json(withItems);
}));

// GET /api/auth/me/tickets  (this customer's own support tickets + replies)
router.get("/me/tickets", requireCustomer, asyncHandler(async (req, res) => {
  const tickets = await db.prepare("SELECT * FROM tickets WHERE email = ? ORDER BY date DESC").all(req.customer.email);
  const withReplies = await Promise.all(tickets.map(async (t) => {
    const replies = await db.prepare("SELECT id, body, created_at FROM ticket_replies WHERE ticket_id = ? ORDER BY created_at").all(t.id);
    return {
      id: t.id,
      subject: t.subject,
      message: t.message,
      status: t.status,
      date: t.date,
      replies: replies.map((r) => ({ id: r.id, body: r.body, createdAt: r.created_at })),
    };
  }));
